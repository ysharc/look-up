...
function connected(port) {
	...
	contentPort.onMessage.addListener(function(message) {
		...
		if (words.length > 4){
			...
			for(var i in words){
				if (words[i])
					getMeaning("http://api.pearson.com/v2/dictionaries/ldoce5/entries?headword=" + words[i] + "&apikey=iXNRYmPiIg8DFaWQsqm3BATO6yKs3qCw&limit=3&audio=pronunciation", words[i], sendMeaning);
			}
		}
	});
}
